...
class TestPseudoMarginalGLMM(unittest.TestCase):
    ...
    def test_pm_hmc_sample(self):
        ...
        beta_samples, re_samples = self.model.pm_hmc_sample(n_iter=10, n_warmup=5, step_size=0.01)
        self.assertEqual(beta_samples.shape[0], 5)
        self.assertEqual(beta_samples.shape[1], self.n_covariates)
        self.assertEqual(re_samples.shape[0], 5)
