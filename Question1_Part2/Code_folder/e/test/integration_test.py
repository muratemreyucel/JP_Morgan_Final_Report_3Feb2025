...
class TestPseudoMarginalGLMM(unittest.TestCase):
    ...
    def test_integration_end_to_end(self):
        """Integration test: Ensure the model runs from data generation to sampling without errors."""
        np.random.seed(42)
        X, y, _, _ = generate_glmm_data(n_subjects=20, n_obs_per_subject=6, n_covariates=5)
        model = PseudoMarginalGLMM(X, y, n_subjects=20, n_covariates=5, n_obs_per_subject=6, n_particles=20)
        beta_samples, re_samples = model.pm_hmc_sample(n_iter=50, n_warmup=10, step_size=0.01)
        
        self.assertEqual(beta_samples.shape[1], 5)
        self.assertEqual(re_samples.shape[1], 20)
        self.assertTrue(np.all(np.isfinite(beta_samples)))
        self.assertTrue(np.all(np.isfinite(re_samples)))
